...
- Akamai Technologies. (2024). *Validate entity tag (ETag)*. **Akamai TechDocs**. https://techdocs.akamai.com/property-mgr/docs/val-entity-tag
- Ed-Fi Alliance. (2025). *Handling optimistic concurrency with ETags*. **Ed‑Fi Data Exchange Standards**. https://docs.ed-fi.org/reference/data-exchange/api-guidelines/design-and-implementation-guidelines/api-implementation-guidelines/handling-optimistic-concurrency-with-etags/
- Fielding, R., & Reschke, J. (2014). *RFC 7232: Hypertext Transfer Protocol (HTTP/1.1): Conditional requests*. **RFC Editor**. https://www.rfc-editor.org/rfc/rfc7232
- Fideloper. (2013). *ETags and optimistic concurrency control*. **Fideloper.com**. https://fideloper.com/etags-and-optimistic-concurrency-control
- MDN Web Docs. (2025). *ETag — HTTP entity tag response header*. **Mozilla Developer Network**. https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/ETag
- Runebook.dev. (2025). *Programming with ETags: Why W/ matters for HTTP caching and concurrency*. **Runebook**. https://runebook.dev/en/docs/http/rfc9110/section-8.8.1
- Schiessl, C. (2024). *Showcasing weak and strong ETag headers with FastAPI*. **BugFactory**. https://bugfactory.io/articles/showcasing-weak-and-strong-etag-headers-with-fastapi/
- Wikipedia contributors. (2025). *HTTP ETag*. **Wikipedia, The Free Encyclopedia**. https://en.wikipedia.org/wiki/HTTP_ETag
